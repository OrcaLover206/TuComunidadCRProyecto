...
async function prepararValidacionUsuarios() {
  try {
    ...
    if (respuesta.ok) {
      ...
      console.warn(
        "No se pudo precargar el JSON de usuarios en registro, se usará solo localStorage.",
      );
    }
    ...
    console.log(
      "Validación de usuarios lista. Total registrados:",
      usuariosParaValidar.length,
    );
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  emailInput.addEventListener("input", () => {
    if (emailInput.value.trim() === "") {
      ...
      mostrarError(
        emailError,
        "El formato de correo no es válido, este debe llevar @ y dominio (.com/.org/etc.).",
      );
    } else {
  ...
  userInput.addEventListener("input", () => {
    ...
    if (username === "") {
      ...
      usuariosParaValidar.some(
        (u) => u.username.toLowerCase() === username.toLowerCase(),
      )
    ) {
      mostrarError(
        userError,
        "Este nombre de usuario ya está en uso, por favor elige otro para registrarte.",
      );
    } else if (username.length <= 2) {
      mostrarError(
        userError,
        "Username demasiado corto, por favor ingrese uno con mayor longitud ",
      );
    } else {
  ...
  registroForm.addEventListener("submit", (evento) => {
    ...
    const usuariosNuevosActuales =
      JSON.parse(localStorage.getItem("usuariosRegistrados")) || [];
    usuariosNuevosActuales.push(nuevoUsuario);
    localStorage.setItem(
      "usuariosRegistrados",
      JSON.stringify(usuariosNuevosActuales),
    );
    ...
    mostrarMensajeGlobal(
      "Registro completado con éxito. Redirigiendo al Inicio...",
      "exito",
    );

    registroForm.reset();

    // Redirigir a la página de Inicio tras 2,7 segundos
    setTimeout(() => {
